...
/// Cline (originally Claude Dev) is a VS Code extension. Its
/// conversation state lives under
/// `<vscode-globalStorage>/saoudrizwan.claude-dev/tasks/<task_id>/`.
/// Cost estimation reuses Claude pricing because Cline routes most
/// traffic through Anthropic's API on behalf of the user.
...
impl ClineAdapter {
    ...
    fn estimate_cost(model: &str, input_tokens: u64, output_tokens: u64) -> f64 {
        // Cline routes through Anthropic by default; pricing matches
        // the Claude adapter's standard tier. Unknown models fall
        // through to Sonnet rates so a stray session isn't free.
...
impl AgentAdapter for ClineAdapter {
    ...
    fn detect_version(&self) -> Option<String> {
        // VS Code extensions don't expose a CLI; the version lives
        // in the manifest at <storage>/../package.json. Best effort:
        // walk up two levels from globalStorage and look for an
        // extensions folder, or just return None.
        None
    }
    ...
    async fn connect(&mut self) -> Result<()> {
        if !self.is_installed() {
            return Err(RimuruError::Adapter(format!(
                "Cline is not installed (expected VS Code extension at {})",
                self.config_path.display()
            )));
        }
        self.connected = true;
        debug!("Connected to Cline adapter");
        Ok(())
    }
    ...
    async fn get_info(&self) -> Result<Agent> {
        let mut agent = Agent::new(AgentType::Cline, "Cline".into());
    ...
    async fn get_sessions(&self) -> Result<Vec<Session>> {
        ...
        for dir in &task_dirs {
            match parse_task_dir(dir, self.agent_id, AgentType::Cline) {
                Ok(mut s) => {
...
impl AdapterCore for ClineAdapter {
