...
fn register_install(iii: &III, kv: &StateKV) {
    ...
    iii.register_function_with(
        RegisterFunctionMessage::with_id("rimuru.plugins.install".to_string()),
        move |input: Value| {
            let kv = kv.clone();
            async move {
...
fn register_uninstall(iii: &III, kv: &StateKV) {
    let kv = kv.clone();
    iii.register_function_with(
        RegisterFunctionMessage::with_id("rimuru.plugins.uninstall".to_string()),
        move |input: Value| {
            let kv = kv.clone();
            async move {
                let plugin_id = require_str(&input, "id")?;
                ...
                if let Some(ref s) = state
                    ...
                    && let Some(pid) = s.pid
                    && let Err(e) = tokio::process::Command::new("kill")
                        .arg(pid.to_string())
                        .output()
                        .await
                {
...
fn register_lifecycle(iii: &III, kv: &StateKV) {
    ...
    iii.register_function_with(
        RegisterFunctionMessage::with_id("rimuru.plugins.start".to_string()),
        move |input: Value| {
    ...
    iii.register_function_with(
        RegisterFunctionMessage::with_id("rimuru.plugins.stop".to_string()),
        move |input: Value| {
